...
async def mark_attendance(request : Request,attendance_request : CreateAttendanceRequest, attendance_params : Annotated[CreateAttendanceQueryParams,Query()],db : Session = Depends(get_sqlite_db),current_user :Staff = Depends(get_current_user)):
    try:
        ...
        if already_marked_attendance:
            raise BadDataException("Attendance is already marked for the student, please update the attendance if you want to change the status")
...
@attendance_router.patch("/update_attendance",status_code=200)
async def update_attendance(request : Request,attendance_request : CreateAttendanceRequest,attendance_params : Annotated[CreateAttendanceQueryParams,Query()],db : Session = Depends(get_sqlite_db),current_user :Staff = Depends(get_current_user)):
    try:
        # check if student exists of the given student_id
        student_to_be_marked = await Student.get_student_by_id(student_id=attendance_params.student_id,db=db)
        ...
        student_standard =  Standard.get_standard_by_id(standard_id=attendance_request.standard_id,db=db)
        if current_user.role==StaffRole.TEACHER and student_standard.class_teacher_id!=current_user.staff_id:
            raise BadDataException("You are not authorized to update attendance for this standard")
        ...
        if not already_marked_attendance:
            raise ResourceNotFoundException("Attendance","student_id and date : ",f"{student_to_be_marked.student_id,attendance_request.date_of_attendance}")
